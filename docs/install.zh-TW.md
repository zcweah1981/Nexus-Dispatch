...
| WebUI | `3030`（Docker Nginx）或 Vite 開發伺服器埠號 | 唯讀可觀測性儀表板，透過 API/SSE 讀取資料，永不直接寫入資料庫。 |
| SQLite | Docker Volume `nexus-sqlite-data:/data` | `DATABASE_URL=file:/data/nexus.db`，由 API 行程內的 Prisma 管理。 |
...
| 症狀 | 檢查命令 | 常見原因 / 修復 |
| --- | --- | --- |
| API 無法啟動 | `docker compose logs nexus-api` / `journalctl -u nexus-dispatch-api -n 100` | 缺少 `DATABASE_URL`、遷移失敗、埠號衝突 |
| 請求返回 401 | 檢查執行環境中的共用驗證 token 和請求標頭 | Bearer token 缺失或不匹配 |
| Daemon 不派單 | `docker compose logs nexus-daemon --since=10m` | 無待處理任務、Agent 不在線上、泳道不匹配、Worker 端點不可達 |
| SQLite 不更新 | `docker compose exec nexus-api npx prisma validate` | Volume 權限問題、`data/nexus.db` 路徑過期 |
| WebUI 空白 | `curl -I http://localhost:3030/` / 瀏覽器主控台 | WebUI 未建構、API/SSE URL 不可達、反向代理未轉發 SSE |
| Telegram 靜默 | 檢查 `AGENT_NOTIFICATIONS` JSON 解析 | Agent 缺少 bot 設定、token/chat_id 錯誤、bot 缺少群組權限 |
| Cron 不執行 | 查詢 `/runtime/projects/:projectId/cronjobs?eligible=true` | 註冊表已暫停/停用、策略不匹配、外部配接器未執行 |
...
`validate-api-deploy.js` 面向原始碼/測試，可在開發者機器安全執行。`health-check.sh` 在空環境或非 Docker/systemd 環境下可能返回 warning 或 critical 狀態——它是部署機器巡檢工具。原始碼層級的交付驗證由 build、test、prisma 和 diff-check 負責。
