...
Nexus Dispatch 是一个多 Agent 编排控制平面——用单一 PM 大脑来派单、追踪、验收跨任意数量异构 AI Agent 的工作。底层基于 API-only、SQLite 驱动、状态机控制的运行时，永远不信任 Worker 自证完成。

---

## 为什么需要 Nexus Dispatch？

大规模使用 AI Agent 时总会撞上同一堵墙：你有十个 Agent，但没有一个大脑在协调它们。任务丢失、重复执行、或者"完成了"却没有证据。聊天频道变成噪音场。没人真正知道什么东西上线了。

Nexus Dispatch 解决这个问题的方式，是成为 **你的 Agent 集群缺失的那个控制平面**：

- **单一 PM 大脑** — 一个 Daemon 负责评估优先级、解析 DAG 依赖、按策略派发任务到正确的 Agent。
- **零信任验收** — Worker 通过 Runtime API 提交 proof、run 和 artifact。状态机说"完成"才算完成。
- **API-only 边界** — 每一次状态流转走 REST。无共享数据库，无 SSH 隧道，无 Agent 直连 DB。
- **人类可读交付** — Telegram 通知由每个 Agent 自己的 bot 发出，Daemon 不代发。追踪 ID 留在数据库里，不进聊天窗口。
...
每个任务严格遵循有限状态机：`created → dispatched → claimed → running → completed | failed | dead-lettered`。没有捷径，任何 Agent 都不能跳过状态或自行标记"已完成"。
...
每个 Agent 用自己的 bot token 发送通知。Daemon 查找 `AGENT_NOTIFICATIONS` 配置，派发到对应 bot。无中心化 bot，无凭据泄露到群聊。
...
**核心不变量：** SQLite 仅在 API Server 进程内可见。Worker、Daemon 和 WebUI 绝不直接操作数据库——它们全部通过 Runtime API 访问。
...
| [docs/assets/](./docs/assets/) | 产品视觉资产：Hero 图 + 架构图（SVG & PNG） |
